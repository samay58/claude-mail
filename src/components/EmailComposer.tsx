...
interface EmailComposerProps {
  ...
  onCancel: () => void;
  onGenerateFromGist?: (gist: string) => void;
  aiSuggestions?: string[];
}

type ComposerField = 'gist' | 'to' | 'subject' | 'body';
...
export function EmailComposer({
  ...
  const [activeField, setActiveField] = useState<ComposerField>(mode === 'compose' ? 'gist' : 'to');
  ...
  useInput((input, key) => {
    ...
    // Field navigation
    if (key.tab) {
      const fields: ComposerField[] = mode === 'compose' ? ['gist', 'to', 'subject', 'body'] : ['to', 'subject', 'body'];
    ...
    if (key.ctrl) {
      ...
      } else if (input === 'g' && onGenerateFromGist && gist.trim()) {
        // Ctrl+G - Generate from gist
        onGenerateFromGist(gist);
      }
    }
  });
    ...
    <Box flexDirection="column" height="100%">
      <Box flexDirection="column" borderStyle="double" borderColor={theme.colors.primary}>
        ...
        <Box flexDirection="column" paddingX={1} paddingY={1}>
          {mode === 'compose' && renderField('gist', 'Quick Gist', gist, setGist, 'What do you want to say? (Ctrl+G to generate)')}
        ...
        <Box borderStyle="single" borderColor={theme.colors.surface} paddingX={1}>
          <Text color={theme.colors.textMuted}>
            [Ctrl+S] Send  [Ctrl+D] Draft  {mode === 'compose' && gist && '[Ctrl+G] Generate  '}[Tab] Navigate  [ESC] Cancel
          </Text>
        </Box>
      </Box>
    </Box>
