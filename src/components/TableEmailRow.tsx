...
export function TableEmailRow({ email, isSelected, index, priorityData }: TableEmailRowProps) {
  ...
  // Format each column to exact width (Total: 74 chars)
  const selectionCol = isSelected ? ' ▶ ' : '   '; // 3 chars
  const priorityCol = padRight(badgeData.priority, 5); // 5 chars (🔴85 + space)
  const senderCol = padRight(truncate(senderName, 10), 10); // 10 chars (+2 from original)
  const subjectCol = padRight(truncate(email.subject, 42), 45); // 45 chars (42 + '...', +20 from original)
  const timeCol = padRight(date, 5); // 5 chars (+1 from original)
