  ...
  private async analyzeEmailHistory(
    ...
    const receivedEmails = this.db['db']
      .prepare(`
        ...
        AND (folder_type = 'inbox' OR folder_type IS NULL OR folder_type = 'other')
        ORDER BY date ASC
      `)
      .all(senderEmail, sixMonthsAgoISO) as EmailRecord[];

    // Get all emails sent to this sender (sent by user)
    // Note: folder_type='sent' is sufficient - all SENT emails are from the user
    // We just need to find emails where the sender we're scoring is a recipient
