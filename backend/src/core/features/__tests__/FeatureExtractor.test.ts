...
describe('FeatureExtractor', () => {
  ...
  describe('OTP Detection (Removed)', () => {
    it('should NOT detect OTP codes (no RFC standard, detection removed)', async () => {
      // OTP detection was removed because there's no RFC standard for it
      // All OTP detection was 100% heuristics with high false positive risk
      const email: EmailRecord = {
        id: 'test-otp-001',
        thread_id: null,
      ...
      // OTP detection removed - field should always be 0
      expect(features.otp_detected).toBe(0);
      expect(features.otp_age_minutes).toBeNull();
    });
  });
  ...
  describe('Feature Importance Calculation', () => {
    ...
    it('should identify positive impact of calendar invite detection', () => {
      // OTP detection was removed (no RFC standard)
      // Testing calendar invite detection instead (RFC 5545)
      const features: MessageFeatures = {
        email_id: 'test-003',
        is_newsletter: 0,
        is_auto_generated: 0,
        ...
        has_list_id: 0,
        has_auto_submitted: 0,
        has_calendar: 1, // Calendar invite detected
        calendar_start_epoch: null,
        otp_detected: 0, // OTP detection removed
        otp_age_minutes: null,
      ...
      const calendarFeature = importance.find(f => f.feature === 'Calendar invite');
      expect(calendarFeature).toBeDefined();
      expect(calendarFeature?.impact).toBe('positive');
    });
  });
