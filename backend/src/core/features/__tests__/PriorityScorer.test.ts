...
describe('PriorityScorer', () => {
  ...
  describe('Negative Signals', () => {
    ...
    it('should NOT penalize OTP codes (OTP detection removed - no RFC standard)', () => {
      const features = createBaselineFeatures();
      // OTP detection was removed because there's no RFC standard
      // otp_detected will always be 0, so this tests the field has no effect
      features.otp_detected = 1; // Even if somehow set, should not affect score
      features.otp_age_minutes = 2;

      const score = scorer.calculatePriority(features);

      // With OTP detection removed, baseline score should remain ~50
