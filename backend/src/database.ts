...
export interface EmailRecord {
  ...
  folder: string;
  folder_type?: string; // 'inbox', 'sent', or 'other'
  labels: string;
  rfc_headers?: string; // JSON string of RFC headers for scoring
  created_at: string;
  updated_at: string;
}
...
class DatabaseManager {
  ...
  private initializeSchema() {
    ...
    // Add folder_type column if it doesn't exist (for SENT folder tracking)
    try {
      this.db.exec(`ALTER TABLE emails ADD COLUMN folder_type TEXT DEFAULT 'inbox'`);
    } catch (error: any) {
    ...
    // Add rfc_headers column if it doesn't exist (for RFC-compliant scoring)
    try {
      this.db.exec(`ALTER TABLE emails ADD COLUMN rfc_headers TEXT DEFAULT '{}'`);
    } catch (error: any) {
      // Column already exists, ignore error
      if (!error.message.includes('duplicate column name')) {
        throw error;
    ...
    this.db.exec('CREATE INDEX IF NOT EXISTS idx_emails_folder_type ON emails(folder_type)');
  ...
  // Email operations
  insertEmail(email: Omit<EmailRecord, 'created_at' | 'updated_at'>): boolean {
    // Check if email already exists
    const existingEmail = this.getEmailById(email.id);
    ...
    const stmt = this.db.prepare(`
      INSERT OR REPLACE INTO emails (
        ...
        is_read, is_starred, is_important, folder, folder_type, labels, rfc_headers
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    ...
    stmt.run(
      ...
      email.folder, email.folder_type || 'inbox', email.labels, email.rfc_headers || '{}'
    );

    // Update contact
    this.upsertContact(email.sender_email, email.sender_name);

    // Return true if this was a new email
    return isNew;
  }
